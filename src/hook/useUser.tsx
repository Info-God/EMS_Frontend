  ...
  const fetchUser = useCallback(async (): Promise<UserType | null> => {
    ...
    try {
      ...
      const { data } = await axiosClient.get<UserType>("/user", {
        headers: {
          // `token` already includes the "Bearer " prefix set at login time —
          // prefixing it again here sent "Bearer Bearer <token>", which Sanctum
          // could never match, so this call always returned 401.
          Authorization: token,
        },
      });
